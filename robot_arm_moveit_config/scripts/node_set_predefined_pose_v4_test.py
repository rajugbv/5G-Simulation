...
# InfluxDB setup
client = InfluxDBClient(url="http://localhost:8086", token="rfLkVFOBSzKxrFLtFV5tz7mo8DxZmTJI6rf6jXmJzhaY-LFzX5Ql539yDktbrTJwNTe3ot0rnvLAlwOtKKlW9A==", org="UofL")
...
class MyRobot:
    ...
    def receive_pose(self):
        ...
        try:
            ...
            print("t1: command fired time from remote computer: "+ time_received)
            t2=time.time()
            print("t2: time received at : " +str( t2))
            frame2=frame[2:]
            return frame2,float(time_received),t2
...
def main():
    ...
    while True:
        ...
        frame,time_r,t2 = arm.receive_pose()  # Use arm's method to receive pose, could be separate if hand needs its own
        if frame:
            start_time_Ac = time.time()
            print ("t3: Actuation start time: "+ str(time.time()))
            arm.set_pose(frame)
            end_time = time.time()
            #finaltime = time_r - end_time
            #actuation_time = start_time_Ac - end_time
            print ("t4: Actuation done: " +str(time.time()))

            communication_delay = t2-time_r
            response_delay = start_time_Ac - t2
            displacement_delay = end_time - start_time_Ac            
            point = Point("pose_data") \
                .tag("pose_type", frame) \
                .field("communication_delay", communication_delay) \
                .field("response_delay", response_delay) \
                .field("displacement_delay",displacement_delay) \
                .time(int(end_time*10**9), WritePrecision.NS)
            
            # Write data to InfluxDB
            write_api.write(bucket="AFRL", record=point)
